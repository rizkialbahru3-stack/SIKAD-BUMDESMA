...
<body>
    <header><img src="{{ public_path('images/logo-bumdesma.png') }}" alt="Logo">
        <div>
            <h1>BUMDESMA LKD TARUB</h1>
            <div>LAPORAN {{ strtoupper(str_replace('-', ' ', $filters['type'])) }}</div>
    ...
    @if ($filters['type'] === 'attendance' || $filters['type'] === 'all')
        ...
        <table>
            ...
            <tbody>
                @foreach ($attendanceSummary as $index => $item)
                    <tr>
                        <td>{{ $index + 1 }}</td>
    ...
    @endif
    @if ($filters['type'] === 'payroll')
        <h2>Laporan Penggajian</h2>
        <div class="summary">
            <div>Total Reward<strong>Rp {{ number_format($payrollSummary['reward'], 0, ',', '.') }}</strong></div>
            <div>Total Potongan<strong>Rp {{ number_format($payrollSummary['deduction'], 0, ',', '.') }}</strong></div>
            <div>Gaji Bersih<strong>Rp {{ number_format($payrollSummary['net'], 0, ',', '.') }}</strong></div>
        </div>
        <table>
            <thead>
                <tr>
            ...
            <tbody>
                @foreach ($payrolls as $index => $item)
                    <tr>
                        ...
                        <td>Rp {{ number_format($item->reward_total, 0, ',', '.') }}</td>
                        <td>Rp
    ...
    @endif
    @if ($filters['type'] === 'reward-punishment')
        <h2>Laporan Reward & Punishment</h2>
        <table>
            <thead>
                <tr>
                    <th>No</th>
    ...
    @endif
    <footer>Laporan Sistem Informasi BUMDESMA LKD TARUB</footer>
</body>
