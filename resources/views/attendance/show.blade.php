...
<div class="mb-4 d-flex flex-wrap gap-2 align-items-center">
    <a class="text-decoration-none me-auto" href="{{ route('attendance.index') }}">
        <i class="bi bi-arrow-left me-2">
        </i>Kembali ke Data Kehadiran</a>
    @if($attendance->check_in_at && ! $attendance->check_in_photo)
        <span class="badge text-bg-info" title="Dicatat manual oleh admin tanpa foto/GPS">Input manual</span>
    @endif
    @if(auth()->user()->isAdmin())
        <a class="btn btn-sm btn-outline-warning" href="{{ route('attendance.corrections.edit', $attendance) }}">
            <i class="bi bi-pencil me-1">
            </i>Koreksi</a>
    @endif
    <span class="badge text-bg-{{ $attendance->status === 'late' ? 'warning' : ($attendance->status === 'present' ? 'success' : 'secondary') }} fs-6">
        {{ ['present' => 'Hadir', 'late' => 'Terlambat', 'leave' => 'Cuti', 'permission' => 'Izin', 'sick' => 'Sakit', 'absent' => 'Alpa'][$attendance->status] ?? ucfirst($attendance->status) }}
    </span>
</div>
...
<div class="alert alert-info d-flex align-items-start gap-2">
    ...
    <div>
        <strong>Keterangan:</strong> {{ $attendance->note }}
    </div>
</div>
@endif
...
<div class="card border-0 shadow-sm mb-4">
    ...
    @if($attendance->employee?->photo)
        <img
            ...
            alt="Foto profil">
    @else
    <span class="employee-avatar" style="width:64px;height:64px;font-size:1.5rem">
        {{ strtoupper(substr($attendance->employee?->display_name ?? '-', 0, 1)) }}
    </span>
    @endif
    <div class="me-auto">
        <h1 class="h5 mb-0">
        ...
        <p class="text-secondary small mb-0">
            {{ $attendance->employee?->employee_code }} · {{ $attendance->employee?->position?->name ?? 'Tanpa jabatan' }}
        </p>
    </div>
...
<div class="row g-4">
    ...
    <div class="col-lg-6">
        ...
        <div class="card-header bg-white py-3">
            <h2 class="h5 mb-0">
                <i class="bi {{ $icon }} me-2 text-{{ $color }}">
                </i>
                {{ $label }}
            </h2>
        </div>
        <div class="card-body">
            ...
            @else
            <div class="alert alert-secondary">Belum ada foto {{ strtolower($label) }}.</div>
            @endif
            <dl class="row small mb-0">
                <dt class="col-4 text-secondary">Waktu</dt>
                <dd class="col-8 fw-semibold">
                    {{ $at?->format('H:i:s').' WIB' ?? '-' }} <span class="text-secondary fw-normal">(server)
                    </span>
                </dd>
                <dt class="col-4 text-secondary">Lokasi</dt>
                <dd class="col-8">
                    {{ $office?->name ?? 'Lokasi kantor belum diatur' }}
                </dd>
                <dt class="col-4 text-secondary">Koordinat</dt>
                <dd class="col-8">
                    {{ ($kind === 'masuk' ? $attendance->check_in_latitude : $attendance->check_out_latitude) !== null ? number_format($kind === 'masuk' ? $attendance->check_in_latitude : $attendance->check_out_latitude, 6).', '.number_format($kind === 'masuk' ? $attendance->check_in_longitude : $attendance->check_out_longitude, 6) : '-' }}
                </dd>
                <dt class="col-4 text-secondary">Akurasi GPS</dt>
                <dd class="col-8">
                    {{ ($kind === 'masuk' ? $attendance->check_in_accuracy : $attendance->check_out_accuracy) !== null ? '±'.($kind === 'masuk' ? $attendance->check_in_accuracy : $attendance->check_out_accuracy).' meter' : '-' }}
                </dd>
                <dt class="col-4 text-secondary">Jarak dari kantor</dt>
                <dd class="col-8">
                    {{ ($kind === 'masuk' ? $attendance->check_in_distance_meters : $attendance->check_out_distance_meters) !== null ? '±'.($kind === 'masuk' ? $attendance->check_in_distance_meters : $attendance->check_out_distance_meters).' meter' : '-' }}
                </dd>
                <dt class="col-4 text-secondary">Status lokasi</dt>
                <dd class="col-8">
...
<div class="card border-0 shadow-sm mt-4">
    <div class="card-header bg-white py-3">
        <h2 class="h5 mb-0">
            <i class="bi bi-map me-2 text-primary">
            </i>Peta Lokasi</h2>
    </div>
    <div class="card-body">
        ...
        <div id="attendanceMap" style="height:380px;border-radius:.9rem;z-index:0">
        </div>
        <div class="d-flex flex-wrap gap-3 mt-3 small text-secondary">
            ...
            @if($attendance->check_in_latitude)
                <span>🟢 Absen masuk (±{{ $attendance->check_in_distance_meters ?? '?' }} m)
                </span>
            @endif
            @if($attendance->check_out_latitude)
                <span>🔵 Absen pulang (±{{ $attendance->check_out_distance_meters ?? '?' }} m)
                </span>
            @endif
        </div>
        @else
        <div
            class="alert alert-secondary mb-0">Belum
            ada
            koordinat
            GPS
            pada
            absensi
            ini
            (data lama sebelum fitur lokasi).</div>
        @endif
    </div>
</div>
@endsection
