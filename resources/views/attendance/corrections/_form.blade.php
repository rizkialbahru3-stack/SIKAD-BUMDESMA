...
<div class="row g-3">
    <div class="col-md-6">
        ...
        <select
            ...
            @foreach($employees as $employee)
                <option
                    value="{{ $employee->id }}"
                    @selected((string) old('employee_id', $attendance->employee_id ?? '') === (string) $employee->id)>
    ...
    <div class="col-md-6">
        ...
        <input
            ...
            value="{{ old('attendance_date', isset($attendance) ? $attendance->attendance_date->format('Y-m-d') : now()->toDateString()) }}"
    ...
    <div class="col-md-6">
        ...
        <input
            ...
            type="time"
            name="check_in"
            value="{{ old('check_in', isset($attendance) && $attendance->check_in_at ? $attendance->check_in_at->format('H:i') : '') }}">
        @error('check_in')<div class="invalid-feedback">{{ $message }}</div>@enderror
    </div>
    <div class="col-md-6">
        <label class="form-label" for="cor_out">Jam pulang <span class="text-secondary">(boleh kosong)
            </span>
        </label>
        <input
            ...
            type="time"
            name="check_out"
            value="{{ old('check_out', isset($attendance) && $attendance->check_out_at ? $attendance->check_out_at->format('H:i') : '') }}">
    ...
    <div class="col-md-6">
        ...
        <select class="form-select @error('status') is-invalid @enderror" id="cor_status" name="status" required>
            @foreach(['present' => 'Hadir', 'late' => 'Terlambat', 'leave' => 'Cuti', 'sick' => 'Sakit', 'absent' => 'Alpa'] as $value => $label)
                <option value="{{ $value }}" @selected(old('status', $attendance->status ?? 'present') === $value)>
                    {{ $label }}
                </option>
            @endforeach
    ...
    <div class="col-md-6">
        ...
        <textarea
            ...
            rows="3"
            maxlength="500"
            placeholder="cth. Lupa absen, HP mati, GPS bermasalah...">
