    ...
    <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-4">
        ...
        @if (auth()->user()->isAdmin())
            <div class="d-flex gap-2">
                <button class="btn btn-success" data-bs-toggle="modal" data-bs-target="#rewardModal">
                    <i
                        class="bi bi-trophy me-2">
                        </i>Tambah reward</button>
                        <button class="btn btn-outline-danger"
                    data-bs-toggle="modal" data-bs-target="#punishmentModal">
                    <i
                        class="bi bi-exclamation-triangle me-2">
                        </i>Tambah punishment</button>
                        </div>
        @endif
    </div>
    @if (auth()->user()->isAdmin())
        <form class="card border-0 shadow-sm mb-4" method="GET">
            <div class="card-body row g-3 align-items-end">
                <div class="col-md-5">
                    <label class="form-label small">Filter karyawan</label>
    ...
    @if (auth()->user()->isAdmin())
        <div class="card border-0 shadow-sm mb-4">
            <div class="card-body row g-3 align-items-end">
                <div class="col-md-8">
                    <h2 class="h6 mb-1">
                        <i class="bi bi-lightning me-2 text-warning">
        ...
        <div class="col-xl-6">
            <div class="card border-0 shadow-sm h-100">
                <div class="card-header bg-white py-3">
                    <h2 class="h5 mb-0">
                        <i class="bi bi-trophy text-success me-2">
                        </i>Riwayat reward</h2>
                </div>
                <div class="list-group list-group-flush">
                            ...
                            <div class="d-flex justify-content-between gap-3">
                                ...
                                <div class="text-end">
                                    <span
                                        ...
                                        {{ $reward->type === 'points' ? $reward->points . ' poin' : 'Rp ' . number_format($reward->amount, 0, ',', '.') }}
                                        </span>
                                    @if (auth()->user()->isAdmin())
                    ...
                    <div class="p-5 text-center text-secondary">
                        <i
                                class="bi bi-trophy fs-2 d-block mb-2">
                                </i>Belum ada reward.</div>
                    @endforelse
                </div>
        ...
        <div class="col-xl-6">
            <div class="card border-0 shadow-sm h-100">
                ...
                <div class="list-group list-group-flush">
                            ...
                            <div class="d-flex justify-content-between gap-3">
                                <div>
                                    ...
                                    @if ($punishment->is_auto)
                                        <span
                                            ...
                                            title="Digenerate otomatis dari keterlambatan">Otomatis</span>
                                    @endif
                                    <small
                                ...
                                <div class="text-end">
                                    <span
                                        ...
                                        {{ $punishment->type === 'salary_deduction' ? 'Rp ' . number_format($punishment->amount, 0, ',', '.') : ($punishment->type === 'points_deduction' && $punishment->points > 0 ? $punishment->points . ' poin' : ucfirst(str_replace('_', ' ', $punishment->type))) }}
                                        </span>
                                    @if (auth()->user()->isAdmin())
                                        <form method="POST"
                    ...
                    <div class="p-5 text-center text-secondary">
                        <i
                                class="bi bi-check-circle fs-2 d-block mb-2">
                                </i>Belum ada punishment.</div>
                    @endforelse
                </div>
            ...
            <div class="modal-dialog">
                <form class="modal-content" method="POST" action="{{ route('recognition.rewards.store') }}">@csrf<div
                    ...
                    <div class="modal-body">
                        ...
                        <input class="form-control mb-3" name="title" placeholder="Judul reward" required>
                        <select
                        ...
                        <div class="row g-2 mb-3">
                            ...
                            <div class="col-6">
                                <input class="form-control" name="points" type="number" min="0"
                                    placeholder="Poin">
                                    </div>
                        </div>
                        <input class="form-control mb-3" name="awarded_at" type="date"
                            value="{{ today()->format('Y-m-d') }}" required>
                        <textarea class="form-control" name="description" rows="3" placeholder="Keterangan">
                        </textarea>
                    </div>
                    <div class="modal-footer">
                        <button class="btn btn-success" type="submit">Simpan reward</button>
                    </div>
                </form>
            </div>
        </div>
            ...
            <div class="modal-dialog">
                <form class="modal-content" method="POST" action="{{ route('recognition.punishments.store') }}">@csrf
                    ...
                    <div class="modal-body">
                        ...
                        <input class="form-control mb-3" name="title" placeholder="Judul punishment"
                            required>
                        ...
                        <input class="form-control mb-3" name="amount" type="number" min="0"
                            placeholder="Nominal potongan">
                            <input class="form-control mb-3" name="issued_at"
                            type="date" value="{{ today()->format('Y-m-d') }}" required>
                        <textarea class="form-control" name="description" rows="3" placeholder="Keterangan">
                        </textarea>
                    </div>
                    <div class="modal-footer">
                        <button class="btn btn-danger" type="submit">Simpan punishment</button>
                    </div>
                </form>
            </div>
        </div>
